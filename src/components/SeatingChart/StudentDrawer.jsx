...
export default function StudentDrawer({ showStudentDrawer, setShowStudentDrawer, unplacedStudents, startDrag, overDrawer }) {
  return (
    <div
      ...
      className={`bg-surface-raised border-slate-800 flex flex-col z-[49] transition-all duration-300 ease-in-out overflow-hidden flex-shrink-0 ${showStudentDrawer ? 'w-64 border-r' : 'w-0 border-r-0'} ${overDrawer ? 'ring-2 ring-inset ring-emerald-400/70' : ''}`}
    >
        ...
        {unplacedStudents.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            {unplacedStudents.map(student => (
              ...
              >
                <div className="w-full h-full rounded-lg flex items-center justify-center bg-emerald-500/20 text-white border border-emerald-500/40 text-[13px] font-bold text-center leading-tight px-1 shadow-md">
